    ...
    <div className={`glass relative rounded-xl p-6 overflow-hidden ${isWinner ? 'border border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : ''}`}>
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 capitalize">Status: {draw.status}</p>
        </div>
        {isWinner && (
              ...
              return (
                <div 
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shadow-md
                    ${isMatch 
                      ? 'bg-gradient-to-br from-emerald-400 to-cyan-500 text-slate-900 ring-2 ring-emerald-400/50' 
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {draw.winning_numbers.map((num, i) => (
                <div key={i} className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold bg-slate-800/50 text-slate-400 border border-slate-700/50">
                  {num}
                </div>
              ))}
            </div>
      ...
      <div className="mt-6 pt-5 border-t border-slate-800 flex justify-between items-end">
        <div>
          <p className="text-sm text-slate-400 mb-0.5">Matches</p>
          <p className="text-xl font-bold text-white">{entry.match_count} / 5</p>
        </div>
