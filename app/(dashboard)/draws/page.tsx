...
export default async function DrawsPage() {
  ...
  return (
    <div className="max-w-4xl space-y-8 animate-fade-in">
      <div>
        <h1 className="text-3xl font-bold text-white tracking-tight">Monthly Draws</h1>
        <p className="text-slate-400 mt-1">View your past draw results and winnings.</p>
      </div>
      ...
      {latestDraw && (
        <div className="glass p-6 rounded-2xl relative overflow-hidden">
          ...
          <h2 className="text-lg font-semibold text-white mb-6">Latest Draw Prize Pool</h2>
          <PrizePoolBar 
      ...
      <div className="flex items-center space-x-2 text-slate-300 bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
      ...
      <div className="space-y-6">
        <h2 className="text-xl font-semibold text-white">Your Draw History</h2>
        
        {!entries || entries.length === 0 ? (
          <div className="glass p-12 rounded-2xl text-center border-dashed">
            <Trophy className="w-16 h-16 text-slate-600 mx-auto mb-4" />
            <h3 className="text-xl font-medium text-white mb-2">No draws yet</h3>
            <p className="text-slate-400">You haven't participated in any draws yet. Ensure your subscription is active and you have scores logged!</p>
          </div>
        ) : (
